...
MetricInfoArtifact = namedtuple("MetricInfoArtifact",
                                ["model_name", "model_object", "train_rmse", "test_rmse","train_mae", 
                                "test_mae","train_mape", "test_mape", "model_rmse", "model_mae", "model_mape" ,"train_accuracy",
                                 "test_accuracy", "model_accuracy", "index_number"])



def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6, base_diff_train_test_acc:float = 0.05) -> MetricInfoArtifact:
    """
    Description:
    ...
    try:
        ...
        for model in model_list:
            ...
            #Calculating mean absolute error on training and testing dataset
            train_mae = mean_absolute_error(y_train, y_train_pred)
            test_mae = mean_absolute_error(y_test, y_test_pred)

            #Calculating mean absolute percentage error on training and testing dataset
            ...
            # Calculating harmonic mean of train and test metrics
            model_rmse = (2 * (train_rmse * test_rmse)) / (train_rmse + test_rmse)
            model_mae = (2 * (train_mae * test_mae)) / (train_mae + test_mae)
            model_mape = (2 * (train_mape * test_mape)) / (train_mape + test_mape)
            ...
            logging.info(f"Model root mean squared error: [{model_rmse}].")
            logging.info(f"Train mean absolute error: [{train_mae}].")
            logging.info(f"Test mean absolute error: [{test_mae}].")
            logging.info(f"Model mean absolute error: [{model_mae}].")
            logging.info(f"Train mean absolute percentage error: [{train_mape}].")
            logging.info(f"Test mean absolute percentage error: [{test_mape}].")
            logging.info(f"Model mean absolute percentage error: [{model_mape}].")


            #if model accuracy is greater than base accuracy and train and test score is within certain thershold
            #we will accept that model as accepted model
            if model_accuracy >= base_accuracy and diff_test_train_acc < base_diff_train_test_acc:
